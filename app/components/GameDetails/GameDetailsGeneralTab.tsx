...
export default function GameDetailsGeneralTab({ game, onRefetch }: GameDetailsGeneralTabProps) {
    ...
    const { basicChanged, genresChanged, releaseBuildChanged, isDirty } = useMemo(() => {
        ...
        // Keep old behavior: only treat release build change as actionable when a build is selected.
    ...
    const validate = () => {
        ...
        if (isNaN(parsedPrice) || parsedPrice < 0) {
            nextErrors.price = "El precio debe ser un número válido (>= 0).";
        }

        if (isNaN(parsedDiscount) || parsedDiscount < 0 || parsedDiscount > 100) {
            nextErrors.discount = "El descuento debe estar entre 0 y 100.";
        }
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex gap-4">
                <Card className="flex-2">
                    <Input.Root id="title" type="text" variant="inside card" value={title} onChange={(v) => {
                        setTitle(v);
                        setSaveSuccess(false);
                    }}>
                ...
                <Card className="flex-1">
                    <Input.Root id="price" type="number" variant="inside card" value={price} onChange={(v) => {
                        setPrice(v);
                        setSaveSuccess(false);
                    }}>
                ...
                <Card className="flex-1">
                    <Input.Root id="discount" type="number" variant="inside card" value={discount} onChange={(v) => {
                        setDiscount(v);
                        setSaveSuccess(false);
                    }}>
